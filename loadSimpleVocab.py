...
def bailout (
        s,			# str. error message to give to user
...
def processArgs ():
        ...
        # Returns: tuple with seven items -- list of str. (input lines),
        #	str.(vocab name), integer (vocab key), str.(J#),
        ...
        try:
                fp = open (file, 'r')
                inputLines = list(map (str.strip, fp.readlines()))
                fp.close()
        except:
...
def parseLines (
        inputLines	# list of str.; input lines from the data file
        ):
...
def createVocabulary (
        vocabName, 	# str. name of vocabulary to be created
...
def getNewTerms (
        ...
        for row in terms:
                ...
                if lowerTerm in oldTerms:
                        existingTerms.append (row)
                elif lowerAbbrev in oldAbbrev:
