...
class DAGLoad:
	...
	def updateClosure (self):
		...
		for (node, children) in closure.items():
			...
			for child in children:
				vocloadlib.nl_sqlog (INSERT_CLOSURE % ( \
	...
	def addNode (self,
		...
		vocloadlib.nl_sqlog (INSERT_NODE % (self.dag_key, node_key,
	...
	def addEdge (self,
		...
		vocloadlib.nl_sqlog (INSERT_EDGE % (
				self.max_edge_key,
				self.dag_key,
				parent_node_key,
				child_node_key,
	...
	def goIncremental (self):
		...
		raise error, 'incremental DAG load not yet implemented'
		return
